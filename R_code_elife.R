...
Tree_m_7_w_outlier<-read.nexus("model_7_w_outlier.nex")
...
model_7_w_outlier<-read.csv2("model_7_w_outlier.csv",header=TRUE,sep=";",dec=",", stringsAsFactors = FALSE)
#standardization 
model_7_w_outlier$ztime_after_mating=scale(model_7_w_outlier$time_after_mating,center=TRUE,scale = TRUE)
model_7_w_outlier$zmaternal_effort=scale(model_7_w_outlier$maternal_effort,center=TRUE,scale = TRUE)
...
Tree_m_7_cons_w_outlier<-consensus(Tree_m_7_w_outlier,p=1,check.labels = TRUE, rooted = TRUE)
Tree_m_7_cons_w_outlier
...
plot(Tree_m_7_cons_w_outlier,cex=1,no.margin = TRUE,label.offset = 1)
...
Tree_m_7_comp_w_outlier <- compute.brlen(Tree_m_7_cons_w_outlier, method="Grafen") 
is.ultrametric(Tree_m_7_comp_w_outlier)
...
is.binary.multiPhylo(Tree_m_7_w_outlier)
is.binary.tree(Tree_m_7_comp_w_outlier)
...
Tree_m_7_comp_w_outlier$node.label<-NULL
comb_model_7_w_outlier<-comparative.data(phy=Tree_m_7_comp_w_outlier,data=model_7_w_outlier,names.col=species,vcv=TRUE,na.omit=FALSE,warn.dropped=TRUE)
str(comb_model_7_w_outlier)
sort(unique(Tree_m_7_comp_w_outlier$tip.label))
sort(unique(model_7_w_outlier$species))
sort(unique(comb_model_7_w_outlier$phy$tip.label))
comb_model_7_w_outlier$dropped$tips
sort(unique(comb_model_7_w_outlier$phy$tip.label))==sort(unique(model_7_w_outlier$species))
...
#The sex that spent the most time in these activities immerged last. 
#This pattern was also influenced by an outlier, a species for which gestation and lactation for females was an extremely long period :  Tachyglossus_aculeatus
#so we made the same model without the outlier, results are presented supplementary materials S3

#best model 7 without outlier :

fit7_w_outlier<-pgls(sex_diff_immerg~
             ztime_after_mating+zmaternal_effort
           ,data=comb_model_7_w_outlier,lambda = "ML")

summary(fit7_w_outlier)

#We tested multicollinearity using the variance inflation factor (VIF < 3) 
#on linear models including the factors of the best models

fit7_w_outliervif<-lm(sex_diff_immerg~
              ztime_after_mating+zmaternal_effort
            ,data=model_7_w_outlier)
vif(fit7_w_outliervif)
